...
const ShareAppModal = ({ open, onClose }: Props) => {
  ...
  const text = language === "ar" ? SHARE_TEXT_AR : SHARE_TEXT_EN;
  const encoded = encodeURIComponent(`${text} ${APP_URL}`);

  const channels = [
    {
      name: "WhatsApp",
      href: `https://wa.me/?text=${encoded}`,
      bg: "bg-[hsl(142_70%_45%)]",
      icon: (
    ...
    {
      name: "Facebook",
      href: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(APP_URL)}&quote=${encodeURIComponent(text)}`,
      bg: "bg-[hsl(221_44%_41%)]",
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white">
    ...
    },
    {
      ...
      bg: "bg-foreground",
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-background">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-5 mt-6">
          {channels.map((c) => (
            ...
            >
              <div className={`w-14 h-14 rounded-full ${c.bg} flex items-center justify-center shadow-md`}>
                {c.icon}
              </div>
